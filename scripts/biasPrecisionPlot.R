...
library(scales); library(raster); library(latex2exp) #can't remember what scale is for...
...
#function reads individual plot simulation tables (10 000 sims with a single value of p) and creates a summary dataframe (one row per value of p with mean values for different parameters)
...
#png("graphics/biasPrecision_RMSE.png", height = 700, width = 700)
par(mfrow = c(2, 2), mar = c(5, 5, 3, 1), xpd = TRUE)

#Stackhousia plot RMSE
plot(x = myPlotPlotsm1$prob.det, y = sqrt(myPlotPlotsm1$mean_sq_error), ylab = "Root Mean Squared Error", xlab = "", ylim = c(0, 2.2), cex.lab = 1.5, cex.axis = 1.5, pch = 16)
# points(x = myPlotPlotsm4$prob.det, y = sqrt(myPlotPlotsm4$mean_sq_error), pch = 16)
# legend(x = 0.8, y = 2, legend = c(expression(1~m^2), expression(4~m^2)), pch=c(1,16), cex = 1.5, bty = 'n')
mtext("probability of detection in plots", line = 3, side = 1, cex = 1.5, at = 1.085)
mtext("A", line = 1, side = 3, at = 0.75, cex = 1.5)
#put AB line at sm dist RMSE
abline(h=sqrt(dsMSE_sm), lty = "dashed", xpd = FALSE)
text(x = 0.65, y = 0.18, labels = "RMSE LTS")
...
#put AB line at sq dist RMSE
abline(h=sqrt(dsMSE_sq), lty = "dashed", xpd = FALSE)
text(x = 0.65, y = 0.008, labels = "RMSE LTS")

#Stackhousia ds RMSE
boxplot(sqrt(dsDatasm$error.sq), cex.axis = 1.5, xaxt = "n", ylim =c(0, 2.2))
abline(h = sqrt(dsMSE_sm), lty = "dashed", xpd = FALSE)
mtext(TeX(r"(Root Squared Error ($\widehat{D}$))"), line = 2.5, side = 2, at = 1.1, cex = 1.3)
mtext("LTS", side = 1, line = 1, at = 1, cex = 1.5)
mtext("C", line = 1, side = 3, at = 1, cex = 1.5)

#Senecio ds RMSE
boxplot(sqrt(dsDatasq$error_sq), cex.axis = 1.5, xaxt = "n", ylim = c(0,0.075))
abline(h = sqrt(dsMSE_sq), lty = "dashed", xpd = FALSE)
mtext("LTS", side = 1, line = 1, at = 1, cex = 1.5)
mtext("D", line = 1, side = 3, at = 1, cex = 1.5)
